...
module.exports = function (N, collectionName) {
  ...
  // Export roles
  //
  ItemWishHistory.statics.roles = roles;
   ...
   * Params:
   *
   * - old_items (Array)  - versions of items before updates
   * - new_items (Array)  - versions of items after updates
   * - meta      (Object) - metadata for this change
   *   - user (ObjectId) - who made this change
   ...
   *
   * This function collapses items if they are made:
   *  - by the same user
   *  - with the same role
   *  - within short period of time (grace period)
   *
   * In order to do this, it fetches previous item version recorded in history
   * and compares all three:
   *  - "new" (latest revision) - from params, item after changes are made
   *  - "old" (2nd latest)      - from params, item before changes are made
   ...
   *    - if "new-old" has no changes (submitted items are identical)
   */
  ItemWishHistory.statics.add = async function addHistory(old_items, new_items, meta) {
    if (!Array.isArray(old_items) && !Array.isArray(new_items)) {
      old_items = [ old_items ];
      new_items = [ new_items ];
    }
    ...
    //
    // Select all history ids for items first, used for:
    //  - selecting last history entry
    //  - counting number of items later
    //
    // (this fetches large numbers of smaller documents, index only)
    //
    let history_ids = await N.models.market.ItemWishHistory.find()
                                .where('item').in(_.map(new_items, '_id'))
                                .select('item _id')
                                .sort('_id')
                                .lean(true);

    //
    // Find number of history entries per item, and last id for each item
    //
    let history = {};

    for (let item of new_items) {
      history[item._id] = { count: 0, last: null };
    }
    ...
    let last_history_entry = _.keyBy(
      await N.models.market.ItemWishHistory.find()
                .where('_id').in(_.map(Object.values(history), 'last').filter(Boolean))
                .lean(true),
      'item'
    );

    let tracked_keys = Object.keys(postSchema);
    let bulk_history = N.models.market.ItemWishHistory.collection.initializeUnorderedBulkOp();

    for (let [ old_item, new_item ] of _.zip(old_items, new_items)) {
      let prev = last_history_entry[new_item._id];
      let old_data = _.pick(old_item, tracked_keys);
      let old_data_str = JSON.stringify(old_data);
      let new_data = _.pick(new_item, tracked_keys);
      ...
      if (prev &&
        ...
        let prev_data = _.pick(prev.data, tracked_keys); // sort keys
      ...
      //  - item ts is within last 5 minutes
      //
      if (!prev &&
          String(new_item.user) === String(meta.user) &&
          meta.role === N.models.market.ItemWishHistory.roles.USER &&
    ...
    let bulk_closed = N.models.market.ItemWishArchived.collection.initializeUnorderedBulkOp();

    for (let new_item of new_items) {
      let bulk;

      if (new_item.st === N.models.market.ItemWish.statuses.OPEN ||
          new_item.ste === N.models.market.ItemWish.statuses.OPEN) {
