...
module.exports = function (N, collectionName) {
  ...
  let UserItemOfferArchivedCount = new Schema({
  ...
  UserItemOfferArchivedCount.index({ user: 1 });
   ...
   * background recount.
   */
  UserItemOfferArchivedCount.statics.get = async function get(user_id, current_user_info) {
    let is_bulk = true;
    ...
    let data = _.keyBy(
      await N.models.market.UserItemOfferArchivedCount.find()
    ...
    if (users_need_recount.length > 0) {
      await N.wire.emit('internal:users.activity.recount',
        users_need_recount.map(u => [ 'market_item_offers_archived', { user_id: u } ])
      );
    }
   ...
   */
  UserItemOfferArchivedCount.statics.inc = async function inc(user_id, { is_hb }) {
    let data = await N.models.market.UserItemOfferArchivedCount.findOneAndUpdate(
      { user: user_id },
      {
        $inc: {
          value: is_hb ? 0 : 1,
    ...
    if (!data) {
      await N.wire.emit('internal:users.activity.recount', [ [ 'market_item_offers_archived', { user_id } ] ]);
    }
  };
   ...
   */
  UserItemOfferArchivedCount.statics.recount = async function recount(user_id) {
    let bulk_data;
    ...
    await N.wire.emit('internal:users.activity.recount', bulk_data.map(user_id => ([
      'market_item_offers_archived',
      { user_id }
    ])));
  };


  N.wire.on('init:models', function emit_init_UserItemOfferArchivedCount() {
    return N.wire.emit('init:models.' + collectionName, UserItemOfferArchivedCount);
  });


  N.wire.on('init:models.' + collectionName, function init_model_UserItemOfferArchivedCount(schema) {
