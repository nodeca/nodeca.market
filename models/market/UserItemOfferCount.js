...
module.exports = function (N, collectionName) {
  ...
  let UserItemOfferCount = new Schema({
  ...
  UserItemOfferCount.index({ user: 1 });
   ...
   * background recount.
   */
  UserItemOfferCount.statics.get = async function get(user_id, current_user_info) {
    let is_bulk = true;
    ...
    let data = _.keyBy(
      await N.models.market.UserItemOfferCount.find()
    ...
    if (users_need_recount.length > 0) {
      await N.wire.emit('internal:users.activity.recount',
        users_need_recount.map(u => [ 'market_item_offers', { user_id: u } ])
      );
    }
   ...
   */
  UserItemOfferCount.statics.inc = async function inc(user_id, { is_hb }) {
    let data = await N.models.market.UserItemOfferCount.findOneAndUpdate(
      { user: user_id },
      {
        $inc: {
          value: is_hb ? 0 : 1,
    ...
    if (!data) {
      await N.wire.emit('internal:users.activity.recount', [ [ 'market_item_offers', { user_id } ] ]);
    }
  };
   ...
   */
  UserItemOfferCount.statics.recount = async function recount(user_id) {
    let bulk_data;
  ...
  N.wire.on('init:models', function emit_init_UserItemOfferCount() {
    return N.wire.emit('init:models.' + collectionName, UserItemOfferCount);
  });


  N.wire.on('init:models.' + collectionName, function init_model_UserItemOfferCount(schema) {
