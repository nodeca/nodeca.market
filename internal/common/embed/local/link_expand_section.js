...
module.exports = function (N) {
  ...
  N.wire.on('internal:common.embed.local', async function embed_market_section_sell(data) {
    if (data.html) return;

    if (data.type !== 'inline') return;

    let match = N.router.matchAll(data.url).reduce((acc, match) => {
      if (match.meta.methods.get === 'market.section.sell') return match;
      return acc;
    }, null);
    ...
    data.html = render(N, 'common.blocks.markup.market_section_sell_link', {
      href:    data.url,
      section
    }, {});
  });
