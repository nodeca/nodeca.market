// Extend `internal:common.abuse_report` to send abuse report for type `MARKET_ITEM_OFFER`
//
// In:
//
...
module.exports = function (N, apiPath) {
  ...
  // Subcall `internal:common.abuse_report` for `MARKET_ITEM_OFFER` content type
  //
  N.wire.on('internal:common.abuse_report', async function market_item_offer_abuse_report_subcall(params) {
    if (params.report.type === N.shared.content_type.MARKET_ITEM_OFFER) {
      params.data = params.data || {};
      await N.wire.emit('internal:common.abuse_report.market_item_offer', params);
    }
  });
  ...
  N.wire.on(apiPath, async function prepare_locals(params) {
    ...
    params.log_templates = {
      body: 'common.abuse_report.market_item_offer.log_templates.body',
      subject: 'common.abuse_report.market_item_offer.log_templates.subject'
    };

    params.email_templates = {
      body: 'common.abuse_report.market_item_offer.email_templates.body',
      subject: 'common.abuse_report.market_item_offer.email_templates.subject'
    };
