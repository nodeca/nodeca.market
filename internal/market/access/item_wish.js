...
// In:
//
// - params.items - array of models.market.ItemWish. Could be plain value
...
module.exports = function (N, apiPath) {
  ...
  //
  N.wire.on('internal:common.access', async function check_market_item_wish_access(access_env) {
    let match = N.router.matchAll(access_env.params.url).reduce(
      (acc, match) => (match.meta.methods.get === 'market.item.wish' ? match : acc),
      null
    );
    ...
    await N.wire.emit('internal:market.access.item_wish', access_env_sub);
  ...
  N.wire.after(apiPath, { priority: 100 }, function allow_read(locals) {
    ...
    if (!Array.isArray(locals.params.items)) {
