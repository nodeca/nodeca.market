// Deliver `MARKET_NEW_OFFER` notification
//
'use strict';
...
module.exports = function (N) {
  ...
  N.wire.on('internal:users.notify.deliver', async function notify_deliver_market_item(local_env) {
    ...
    // Fetch item
    //
    let item = await N.models.market.ItemOffer.findById(local_env.src).lean(true);
    if (!item) return;
    ...
    let subscriptions = await N.models.users.Subscription.find()
                                  ...
                                  .where('to_type').equals(N.shared.content_type.MARKET_SECTION_OFFER)
                                  .lean(true);
    ...
    for (let user_id of user_ids) {
      ...
      let subject = N.i18n.t(locale, 'users.notify.market_new_offer.subject', {
        project_name: general_project_name,
        section_title: section.title
      });

      let url = N.router.linkTo('market.item.buy', {
        section_hid: section.hid,
        item_hid: item.hid
      });

      let unsubscribe = N.router.linkTo('market.section.buy.unsubscribe', {
        section_hid: section.hid
      });

      let text = render(N, 'users.notify.market_new_offer', {
        title: item.title,
        post_html: item.html,
        url,
        unsubscribe
