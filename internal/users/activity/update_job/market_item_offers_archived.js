...
module.exports = function (N, apiPath) {
  ...
  N.wire.on(apiPath, async function activity_fetch_market_item_offers_archived({ user_id }) {
    // check that user exists
    let user = await N.models.users.User.findById(user_id).lean(true);
    if (!user) return;

    let results = await N.models.market.ItemOfferArchived
                            .where('user').equals(user._id)
                            .where('st').equals(N.models.market.ItemOffer.statuses.CLOSED)
                            .countDocuments();

    let results_hb = await N.models.market.ItemOfferArchived
                               .where('user').equals(user._id)
                               .where('st').equals(N.models.market.ItemOffer.statuses.HB)
                               .countDocuments();

    await N.models.market.UserItemOfferArchivedCount.replaceOne(
      { user: user._id },
