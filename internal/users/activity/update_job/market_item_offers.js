...
module.exports = function (N, apiPath) {
  ...
  N.wire.on(apiPath, async function activity_fetch_market_item_offers({ user_id }) {
    // check that user exists
    let user = await N.models.users.User.findById(user_id).lean(true);
    if (!user) return;

    let results = await N.models.market.ItemOffer
                            .where('user').equals(user._id)
                            .where('st').equals(N.models.market.ItemOffer.statuses.OPEN)
                            .countDocuments();

    let results_hb = await N.models.market.ItemOffer
                               .where('user').equals(user._id)
                               .where('st').equals(N.models.market.ItemOffer.statuses.HB)
                               .countDocuments();

    await N.models.market.UserItemOfferCount.replaceOne(
      { user: user._id },
