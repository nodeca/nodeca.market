...
function load(start, direction) {
  return N.io.rpc('market.all.buy.list.by_range', {
    start,
...
function on_list_scroll(item, index, item_offset) {
  ...
  update_url = update_url || _.debounce((item, index, item_offset) => {
    ...
    if (pageState.current_offset !== index) {
      ...
      href = N.router.linkTo('market.all.buy', { $query });
...
N.wire.on('navigate.done:' + module.apiPath, function page_setup(data) {
  let pagination    = N.runtime.page_data.pagination,
      last_item_hid = $('.market-all-buy').data('last-item-hid');
  ...
  //
  if (!scroll_done) {
    if (pagination.chunk_offset > 1 && $('.market-all-buy__item-list').length) {
      $window.scrollTop($('.market-all-buy__item-list').offset().top - navbar_height);
    } else {
      $window.scrollTop(0);
    }
    scroll_done = true;
  ...
  scrollable_list = new ScrollableList({
    N,
    list_selector:               '.market-all-buy__item-list',
    item_selector:               '.market-list-item-offer',
    placeholder_top_selector:    '.market-all-buy__loading-prev',
    placeholder_bottom_selector: '.market-all-buy__loading-next',
    get_content_id:              item => $(item).data('item-id'),
    load,
    reached_top:                 pagination.chunk_offset === 0,
    reached_bottom:              last_item_hid === $('.market-all-buy__item-list > :last').data('item-hid'),
...
N.wire.once('navigate.done:' + module.apiPath, function market_all_init_handlers() {
  ...
  N.wire.on(module.apiPath + ':nav_to_end', function navigate_to_end() {
    ...
    return N.wire.emit('navigate.to', {
      ...
      params: {
        $query: {
          from: String($('.market-all-buy').data('last-item-hid'))
        }
      }
    });
  });
...
N.wire.on('navigate.done:' + module.apiPath, function market_load_previously_selected_items() {
  selected_items_key = `market_all_buy_selected_items_${N.runtime.user_hid}`;
...
N.wire.once('navigate.done:' + module.apiPath, function market_item_selection_init() {
  ...
  N.wire.on(module.apiPath + ':delete_many', function delete_many() {
    ...
    return Promise.resolve()
      .then(() => N.wire.emit('market.all.buy.item_delete_many_dlg', params))
      .then(() => {
        let request = {
  ...
  N.wire.on(module.apiPath + ':move_many', function move_many() {
    ...
    return Promise.resolve()
      .then(() => N.wire.emit('market.all.buy.item_move_many_dlg', params))
      .then(() => {
        let request = {
