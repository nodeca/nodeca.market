...
N.wire.once('navigate.done:' + module.apiPath, function page_once() {
  ...
  N.wire.before('users.dialog.create:begin', function dialog_create_extend_market_items(params) {
    ...
    if (!/^market_section_item:/.test(params.ref)) return; // not our data

    let [ , section_hid, item_hid ] = params.ref.split(':');
    let title = $(`#item${item_hid} .market-section-item__title-text`).text();
    let href  = N.router.linkTo('market.item', { section_hid, item_hid });
