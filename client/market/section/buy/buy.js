...
function load(start, direction) {
  return N.io.rpc('market.section.buy.list.by_range', {
...
N.wire.on('navigate.done:' + module.apiPath, function page_setup(data) {
  ...
  if (!scroll_done) {
    if (pagination.chunk_offset > 1 && $('.market-section-buy__item-list').length) {
      $window.scrollTop($('.market-section-buy__item-list').offset().top - navbar_height);
    } else {
      $window.scrollTop(0);
    }
    scroll_done = true;
  ...
  scrollable_list = new ScrollableList({
    N,
    list_selector:               '.market-section-buy__item-list',
    item_selector:               '.market-list-item-offer',
    placeholder_top_selector:    '.market-section-buy__loading-prev',
    placeholder_bottom_selector: '.market-section-buy__loading-next',
    get_content_id:              item => $(item).data('item-id'),
    load,
    reached_top:                 pagination.chunk_offset === 0,
    reached_bottom:              last_item_hid === $('.market-section-buy__item-list > :last').data('item-hid'),
...
N.wire.once('navigate.done:' + module.apiPath, function market_item_selection_init() {
  ...
  N.wire.on(module.apiPath + ':delete_many', function delete_many() {
    ...
    return Promise.resolve()
      .then(() => N.wire.emit('market.section.buy.item_delete_many_dlg', params))
      .then(() => {
        let request = {
  ...
  N.wire.on(module.apiPath + ':move_many', function move_many() {
    ...
    return Promise.resolve()
      .then(() => N.wire.emit('market.section.buy.item_move_many_dlg', params))
      .then(() => {
        let request = {
