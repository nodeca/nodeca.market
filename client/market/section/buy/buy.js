...
N.wire.on('navigate.done:' + module.apiPath, function page_setup(data) {
  let pagination    = N.runtime.page_data.pagination,
      last_item_hid = $('.market-section-buy').data('last-item-hid');
  ...
  pageState.reached_end        = last_item_hid === $('.market-section-buy__item-list > :last').data('item-hid');
  ...
  if (data.state && typeof data.state.hid !== 'undefined' && typeof data.state.offset !== 'undefined') {
    ...
    if (el.length) {
      ...
      el.addClass('market-section-buy-item__m-highlight');
      return;
    }
  }
  ...
  //
  if (pagination.chunk_offset > 1 && $('.market-section-buy__item-list').length) {
    $window.scrollTop($('.market-section-buy__item-list').offset().top - $('.navbar').height());
...
N.wire.on('navigate.done:' + module.apiPath, function location_updater_init() {
  if ($('.market-section-buy__item-list').length === 0) return;

  locationScrollHandler = _.debounce(function update_location_on_scroll() {
    let items         = document.getElementsByClassName('market-section-buy-item');
    ...
    if (pageState.current_offset !== offset) {
      ...
      href = N.router.linkTo('market.section.buy', {
...
function reset_loading_placeholders() {
  let prev = $('.market-section-buy__loading-prev');
  let next = $('.market-section-buy__loading-next');
...
N.wire.once('navigate.done:' + module.apiPath, function market_section_init_handlers() {
  ...
  N.wire.on(module.apiPath + ':load_prev', function load_prev_page() {
    ...
    N.io.rpc('market.section.buy.list.by_range', {
      section_hid: pageState.hid,
      start:       last_item_id,
      ...
      let old_height = $('.market-section-buy__item-list').height();

      // render & inject item list
      let $result = $(N.runtime.render('market.blocks.items_buy_list', res));

      return N.wire.emit('navigate.update', {
        $:       $result,
        locals:  res,
        $before: $('.market-section-buy__item-list > :first')
      }).then(() => {
        ...
        if (cut_count > CUT_ITEMS_MAX - CUT_ITEMS_MIN) {
          ...
          if (item.getBoundingClientRect().top > $window.height() + 600) {
            ...
            pageState.bottom_marker = $('.market-section-buy-item:last').data('item-id');
      ...
      return N.wire.emit('navigate.update', {
        ...
        if (cut_count > CUT_ITEMS_MAX - CUT_ITEMS_MIN) {
          ...
          if (item.getBoundingClientRect().bottom < -600) {
            ...
            // Update range for the next time we'll be doing prefetch
            pageState.top_marker = $('.market-section-buy-item:first').data('item-id');

            // update scroll so it would point at the same spot as before
            $window.scrollTop(old_scroll + $('.market-section-buy__item-list').height() - old_height);
            pageState.first_offset += old_length - document.getElementsByClassName('market-section-buy-item').length;
  ...
  N.wire.on(module.apiPath + ':nav_to_end', function navigate_to_end() {
    ...
    return N.wire.emit('navigate.to', {
      ...
      params: {
        section_hid: pageState.hid,
        $query: {
          from: String($('.market-section-buy').data('last-item-hid'))
        }
      }
    });
  });
