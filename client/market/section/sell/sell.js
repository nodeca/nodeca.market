...
N.wire.on('navigate.done:' + module.apiPath, function page_setup(data) {
  let pagination    = N.runtime.page_data.pagination,
      last_item_hid = $('.market-section-sell').data('last-item-hid');
  ...
  pageState.reached_end        = last_item_hid === $('.market-section-sell__item-list > :last').data('item-hid');
  pageState.prev_loading_start = 0;
  pageState.next_loading_start = 0;
  pageState.top_marker         = $('.market-section-sell').data('top-marker');
  pageState.bottom_marker      = $('.market-section-sell').data('bottom-marker');
  ...
  if (data.state && typeof data.state.hid !== 'undefined' && typeof data.state.offset !== 'undefined') {
    ...
    if (el.length) {
      ...
      el.addClass('market-section-sell-item__m-highlight');
      return;
    }
  }
  ...
  //
  if (pagination.chunk_offset > 1 && $('.market-section-sell__item-list').length) {
    $window.scrollTop($('.market-section-sell__item-list').offset().top - $('.navbar').height());
...
N.wire.on('navigate.done:' + module.apiPath, function location_updater_init() {
  if ($('.market-section-sell__item-list').length === 0) return;

  locationScrollHandler = _.debounce(function update_location_on_scroll() {
    let items         = document.getElementsByClassName('market-section-sell-item');
    ...
    if (pageState.current_offset !== offset) {
      ...
      href = N.router.linkTo('market.section.sell', {
...
function reset_loading_placeholders() {
  let prev = $('.market-section-sell__loading-prev');
  let next = $('.market-section-sell__loading-next');
...
N.wire.once('navigate.done:' + module.apiPath, function market_section_init_handlers() {
  ...
  N.wire.on(module.apiPath + ':load_prev', function load_prev_page() {
    ...
    N.io.rpc('market.section.sell.list.by_range', {
      section_hid: pageState.hid,
      start:       last_item_id,
      ...
      return N.wire.emit('navigate.update', {
        $:       $result,
        locals:  res,
        $before: $('.market-section-sell__item-list > :first')
      }).then(() => {
        // update scroll so it would point at the same spot as before
        $window.scrollTop($window.scrollTop() + $('.market-section-sell__item-list').height() - old_height);
        ...
        if (cut_count > CUT_ITEMS_MAX - CUT_ITEMS_MIN) {
          ...
          if (item.getBoundingClientRect().top > $window.height() + 600) {
            ...
            pageState.bottom_marker = $('.market-section-sell-item:last').data('item-id');
  ...
  N.wire.on(module.apiPath + ':load_next', function load_next_page() {
    ...
    N.io.rpc('market.section.sell.list.by_range', {
      ...
      pageState.first_offset  = res.pagination.chunk_offset - $('.market-section-sell-item').length;
      ...
      return N.wire.emit('navigate.update', {
        $:      $result,
        locals: res,
        $after: $('.market-section-sell__item-list > :last')
      }).then(() => {
        //
        // Limit total amount of posts in DOM
        //
        ...
        if (cut_count > CUT_ITEMS_MAX - CUT_ITEMS_MIN) {
          ...
          if (item.getBoundingClientRect().bottom < -600) {
            ...
            // Update range for the next time we'll be doing prefetch
            pageState.top_marker = $('.market-section-sell-item:first').data('item-id');

            // update scroll so it would point at the same spot as before
            $window.scrollTop(old_scroll + $('.market-section-sell__item-list').height() - old_height);
            pageState.first_offset += old_length - document.getElementsByClassName('market-section-sell-item').length;
  ...
  N.wire.on(module.apiPath + ':nav_to_end', function navigate_to_end() {
    ...
    return N.wire.emit('navigate.to', {
      ...
      params: {
        section_hid: pageState.hid,
        $query: {
          from: String($('.market-section-sell').data('last-item-hid'))
        }
      }
    });
  });
  ...
  N.wire.before('users.dialog.create:begin', function dialog_create_extend_market_items(params) {
    ...
    if (!/^market_section_sell_item:/.test(params.ref)) return; // not our data

    let [ , section_hid, item_hid ] = params.ref.split(':');
    let title = $(`#item${item_hid} .market-section-sell-item__title-text`).text();
