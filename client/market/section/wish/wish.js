...
function load(start, direction) {
  return N.io.rpc('market.section.wish.list.by_range', {
...
N.wire.on('navigate.done:' + module.apiPath, function page_setup(data) {
  ...
  if (!scroll_done) {
    if (pagination.chunk_offset > 1 && $('.market-section-wish__item-list').length) {
      $window.scrollTop($('.market-section-wish__item-list').offset().top - navbar_height);
    } else {
      $window.scrollTop(0);
    }
    scroll_done = true;
  ...
  scrollable_list = new ScrollableList({
    N,
    list_selector:               '.market-section-wish__item-list',
    item_selector:               '.market-list-item-wish',
    placeholder_top_selector:    '.market-section-wish__loading-prev',
    placeholder_bottom_selector: '.market-section-wish__loading-next',
    get_content_id:              item => $(item).data('item-id'),
    load,
    reached_top:                 pagination.chunk_offset === 0,
    reached_bottom:              last_item_hid === $('.market-section-wish__item-list > :last').data('item-hid'),
...
function updateToolbar() {
  $('.market-section-wish__toolbar-controls')
...
N.wire.once('navigate.done:' + module.apiPath, function market_section_init_handlers() {
  ...
  N.wire.on(module.apiPath + ':subscription', function section_subscription(data) {
    ...
    return Promise.resolve()
      .then(() => N.wire.emit('market.section.wish.subscription', params))
      .then(() => N.io.rpc('market.section.wish.change_subscription', { section_hid: hid, type: params.subscription }))
      .then(() => {
  ...
  N.wire.on(module.apiPath + ':mark_read', function mark_read(data) {
    return N.io.rpc('market.section.wish.mark_read', { hid: data.$this.data('section-hid') })
...
N.wire.on('navigate.done:' + module.apiPath, function market_load_previously_selected_items() {
  selected_items_key = `market_section_wish_selected_items_${N.runtime.user_hid}_${pageState.hid}`;
