...
module.exports = function (N, apiPath) {
  ...
  // Using different sanitizer for items here,
  ...
  N.wire.on(apiPath, async function get_item_history(env) {
    ...
    env.res.history_data = _.map(history, 'data')
                            .concat([ env.data.item ])
                            .map(sanitize_item);

    env.data.users = (env.data.users || []).concat(_.map(env.res.history_meta, 'user'));
  });
  ...
  N.wire.after(apiPath, async function fetch_sections(env) {
    let sections = await N.models.market.Section.find()
                             .where('_id').in(_.uniq(env.res.history_data.map(h => String(h.section))))
                             .lean(true);
  ...
  N.wire.after(apiPath, async function fetch_locations(env) {
    let locations = env.res.history_data.map(i => i.location).filter(Boolean);
