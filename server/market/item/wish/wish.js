...
module.exports = function (N, apiPath) {
  ...
  N.wire.after(apiPath, async function fill_breadcrumbs(env) {
    if (env.data.item_is_archived) {
      ...
      await N.wire.emit('internal:market.breadcrumbs_fill', { env, parents, wish: true });
    }
  });
  ...
  N.wire.after(apiPath, function update_view_counter(env) {
    ...
    N.redis.time(function (err, time) {
      ...
      N.redis.zscore('views:market_item_wish:track_last', key, function (err, old_score) {
        ...
        N.redis.zadd('views:market_item_wish:track_last', score, key, function (err) {
          if (err) return;

          N.redis.hincrby('views:market_item_wish:count', String(env.data.item._id), 1, function () {});
        });
      });
    });
  });
