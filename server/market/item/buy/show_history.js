...
module.exports = function (N, apiPath) {
  ...
  N.wire.on(apiPath, async function get_item_history(env) {
    ...
    env.res.history.push({
      ...
      price:       env.data.item.price,
      md:          env.data.item.md,
      barter_info: env.data.item.barter_info,
      delivery:    env.data.item.delivery,
      is_new:      env.data.item.is_new,
      location:    env.data.item.location,
      files:       env.data.item.files,
      user:        previous_user,
      ts:          previous_ts
    });
  });
