...
module.exports = function (N, apiPath) {
  ...
  N.wire.before(apiPath, async function fetch_item(env) {
    ...
    await N.wire.emit('internal:market.access.item', access_env);
  ...
  N.wire.before(apiPath, function redirect_to_correct_section(env) {
    if (env.data.section.hid !== env.params.section_hid) {
      throw {
        code: N.io.REDIRECT,
        head: {
          Location: N.router.linkTo('market.item', {
  ...
  N.wire.on(apiPath, async function market_item(env) {
    ...
    env.res.item    = await sanitize_item(N, env.data.item, env.user_info);
  });
  ...
  N.wire.after(apiPath, function update_view_counter(env) {
    ...
    N.redis.time(function (err, time) {
      ...
      N.redis.zscore('views:market_item:track_last', key, function (err, old_score) {
        if (err) return;
        ...
        N.redis.zadd('views:market_item:track_last', score, key, function (err) {
          if (err) return;

          N.redis.hincrby('views:market_item:count', String(env.data.item._id), 1, function () {});
        });
      });
    });
  });
