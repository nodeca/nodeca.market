// Flush view counters from `views:market_item:count` in redis
...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_market_item_views_update() {
    const task_name = 'market_item_views_update';
    ...
    N.queue.registerTask({
      ...
      async process() {
        try {
          ...
          try {
            await N.redis.renameAsync('views:market_item:count', 'views:market_item:count_tmp');
          } catch (__) {}

          let items = await N.redis.hgetallAsync('views:market_item:count_tmp');

          await N.redis.delAsync('views:market_item:count_tmp');
          ...
          await N.redis.zremrangebyscoreAsync('views:market_item:track_last', '-inf', score);
        } catch (err) {
