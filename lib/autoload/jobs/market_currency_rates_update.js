...
module.exports = function (N) {
  ...
  N.wire.on('init:jobs', function register_market_currency_rates_update() {
    ...
    N.queue.registerTask({
      ...
      removeDelay: 3600,
      async process(ids) {
        // this method recalculate base currency price for all items in a chunk
        // (updates mongo and search)
        let items = await N.models.market.ItemOffer.find()
                                .where('_id').in(ids)
                                .lean(true);

        // filter out items that don't have a price (nothing to update)
        items = items.filter(i => i.price);

        if (!items.length) return;

        let currencies = _.uniq(items.map(i => i.price.currency).filter(Boolean));
        let currency_rates = {};
