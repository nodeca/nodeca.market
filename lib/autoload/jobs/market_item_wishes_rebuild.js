...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_market_item_wishes_rebuild() {
    // Iterator
    //
    N.queue.registerTask({
      name: 'market_item_wishes_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'market_item_wishes_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(items.map(p => String(p._id)), ITEMS_PER_CHUNK)
                      .map(ids => N.queue.market_item_wishes_rebuild_chunk(ids));
    ...
    //
    N.queue.registerTask({
      name: 'market_item_wishes_rebuild_chunk',
      pool: 'hard',
      removeDelay: 3600,
      async process(ids) {
        ...
        await N.wire.emit('internal:market.item_wish_rebuild', ids);
