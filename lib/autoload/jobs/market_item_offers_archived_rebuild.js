...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_market_item_offers_archived_rebuild() {
    // Iterator
    //
    N.queue.registerTask({
      name: 'market_item_offers_archived_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'market_item_offers_archived_rebuild',
      ...
      async iterate(state) {
        ...
        // Fetch items _id
        //
        let query = N.models.market.ItemOfferArchived.find()
        ...
        let chunks = _.chunk(items.map(p => String(p._id)), ITEMS_PER_CHUNK)
                      .map(ids => N.queue.market_item_offers_archived_rebuild_chunk(ids));
      ...
      async init() {
        let query = N.models.market.ItemOfferArchived.count();

        if (this.args.length < 1 || !this.args[0]) {
          // if no min _id
          let min_item = await N.models.market.ItemOfferArchived.findOne()
                                  .select('_id')
                                  .sort('_id')
                                  .lean(true);
        ...
        if (this.args.length < 2 || !this.args[1]) {
          // if no max _id
          let max_item = await N.models.market.ItemOfferArchived.findOne()
                                   .select('_id')
                                   .sort('-_id')
                                   .lean(true);
    ...
    //
    N.queue.registerTask({
      name: 'market_item_offers_archived_rebuild_chunk',
      pool: 'hard',
      removeDelay: 3600,
      async process(ids) {
        ...
        await N.wire.emit('internal:market.item_offer_archived_rebuild', ids);
