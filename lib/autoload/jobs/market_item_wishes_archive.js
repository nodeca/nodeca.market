  ...
  N.wire.on('init:jobs', function register_market_item_wishes_archive() {
    ...
    N.queue.registerTask({
      ...
      async process() {
        ...
        await bulk.execute();
        await N.models.market.ItemWish.deleteMany({ _id: { $in: _.map(docs, '_id') } });
        ...
        await N.models.market.ItemWishHistory.add(
          ...
          {
            user: bot && bot._id,
            role: N.models.market.ItemWishHistory.roles.TASK
          }
        );
        ...
        // update user cache
        let users = _.uniq(_.map(docs, 'user').map(String));
