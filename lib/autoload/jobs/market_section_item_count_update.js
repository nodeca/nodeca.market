  ...
  N.wire.on('init:jobs', function register_market_section_item_count_update() {
    N.queue.registerTask({
      ...
      async process(section_str_id) {
        ...
        let wishes_visible = await N.models.market.ItemWish.count()
                                         .where('section').equals(section_id)
                                         .where('st').equals(N.models.market.ItemWish.statuses.VISIBLE);

        let wishes_hb = await N.models.market.ItemWish.count()
                                    .where('section').equals(section_id)
        ...
        let children_cnt = await N.models.market.Section
                                     ...
                                     .group({
                                       ...
                                       wish_count_hb: { $sum: '$cache.wish_count_hb' }
                                     })
                                     .exec();
