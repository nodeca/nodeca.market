...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_market_item_offers_active_rebuild() {
    // Iterator
    //
    N.queue.registerTask({
      name: 'market_item_offers_active_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'market_item_offers_active_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(items.map(p => String(p._id)), ITEMS_PER_CHUNK)
                      .map(ids => N.queue.market_item_offers_active_rebuild_chunk(ids));
    ...
    //
    N.queue.registerTask({
      name: 'market_item_offers_active_rebuild_chunk',
      pool: 'hard',
      removeDelay: 3600,
      async process(ids) {
        ...
        await N.wire.emit('internal:market.item_offer_active_rebuild', ids);
