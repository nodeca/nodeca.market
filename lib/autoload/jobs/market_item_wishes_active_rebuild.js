...
module.exports = function (N) {
  N.wire.on('init:jobs', function register_market_item_wishes_active_rebuild() {
    // Iterator
    //
    N.queue.registerTask({
      name: 'market_item_wishes_active_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'market_item_wishes_active_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(items.map(p => String(p._id)), ITEMS_PER_CHUNK)
                      .map(ids => N.queue.market_item_wishes_active_rebuild_chunk(ids));
      ...
      async init() {
        let query = N.models.market.ItemWish.countDocuments();

        if (this.args.length < 1 || !this.args[0]) {
          // if no min _id
          let min_item = await N.models.market.ItemWish.findOne()
                                  .select('_id')
                                  .sort('_id')
                                  .lean(true);
        ...
        if (this.args.length < 2 || !this.args[1]) {
          // if no max _id
          let max_item = await N.models.market.ItemWish.findOne()
                                   .select('_id')
                                   .sort('-_id')
                                   .lean(true);
    ...
    //
    N.queue.registerTask({
      name: 'market_item_wishes_active_rebuild_chunk',
      pool: 'hard',
      removeDelay: 3600,
      async process(ids) {
        ...
        await N.wire.emit('internal:market.item_wish_active_rebuild', ids);
