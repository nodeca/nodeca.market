...
module.exports = function (N) {
  ...
  N.wire.on('init:jobs', function register_market_item_wishes_search_rebuild() {

    N.queue.registerTask({
      name: 'market_item_wishes_search_rebuild',
      pool: 'hard',
      baseClass: Queue.IteratorTemplate,
      taskID: () => 'market_item_wishes_search_rebuild',
      ...
      async iterate(state) {
        ...
        let chunks = _.chunk(items.map(p => String(p._id)), ITEMS_PER_CHUNK)
                      .map(ids => N.queue.market_item_wishes_search_update_by_ids(ids, { shadow: true }));
