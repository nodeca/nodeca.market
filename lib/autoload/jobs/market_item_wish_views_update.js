    ...
    N.queue.registerTask({
      ...
      async process() {
        try {
          ...
          try {
            await N.redis.rename('views:market_item_wish:count', 'views:market_item_wish:count_tmp');
          } catch (__) {}

          let items = await N.redis.hgetall('views:market_item_wish:count_tmp');

          await N.redis.del('views:market_item_wish:count_tmp');

          if (!_.isEmpty(items)) {
            let bulk = N.models.market.ItemWish.collection.initializeUnorderedBulkOp();
            let bulk_closed = N.models.market.ItemWish.collection.initializeUnorderedBulkOp();
          ...
          await N.redis.zremrangebyscore('views:market_item_wish:track_last', '-inf', score);
        } catch (err) {
