...
const docid_item_requests = require('nodeca.market/lib/search/docid_item_requests');
...
module.exports = function (N) {
  ...
  N.wire.on('init:jobs', function register_market_item_requests_search_update_by_ids() {

    N.queue.registerTask({
      name: 'market_item_requests_search_update_by_ids',
      pool: 'hard',
      removeDelay: 3600,
      async process(ids, options = {}) {
        ...
        for (let idx = 0; idx < items.length; idx++) {
          ...
          let visible = item.st === N.models.market.ItemRequest.statuses.VISIBLE;

          values.push('(?,?,?,?,?,?,?,?,?,?,?)');

          args.push(
            // id
            docid_item_requests(N, item.hid),
            // title
            item.title,
            // content
            item.html,
        ...
        let query = `
          REPLACE INTO market_item_requests
