...
//
// - N
// - topics - array of models.market.ItemRequest or models.market.ItemRequest. Could be plain value
...
module.exports = async function (N, items, user_info) {
  ...
  res = res.map(item => {
    if (item.st === N.models.market.ItemRequest.statuses.HB && !can_see_hellbanned) {
      item.st = item.ste;
      delete item.ste;
    }
