...
//
// - N
// - topics - array of models.market.ItemOffer or models.market.ItemRequest. Could be plain value
...
module.exports = async function (N, items, user_info) {
  ...
  res = res.map(item => {
    // can't easily distinguish offer and request objects here,
    // but HB should be the same value in ItemOffer and ItemRequest
    if ((item.st === N.models.market.ItemOffer.statuses.HB || N.models.market.ItemRequest.statuses.HB) &&
        !can_see_hellbanned) {
